...
for fl in os.listdir(dhcb_path):
	...
	try:
		...
		#find out all the rows that have null values in them
		null = set()
		for ind_list in inds:
			for col in ind_list:
				null.update(np.where(df.iloc[:,col].isnull())[0])
		
		null_dict[fl] = len(null)

		full_rows = set([i for i in range(len(df))])
		valid_rows = full_rows - null
		
		print()
		print("The number of rows containing null values:", len(null))
		print("The number of rows in the original dataframe:", len(df))
		df = df.iloc[list(valid_rows), :]
		print("The number of rows in the valid dataframe:", len(df))
		print()
...
pickle.dump(null_dict, open('null_villages_per_state.pickle', 'wb'))
